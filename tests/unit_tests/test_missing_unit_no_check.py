...
def test_missing_road_key(step):
    """
    If ctx["parsed"] exists but has no "road" key, ctx.get("parsed", {}).get("road") is None,
    so validate_status should be set.
    """
    ctx = {"parsed": {"house_number": "123"}}
    updated = step(ctx.copy())
    assert updated["validate_status"] == ValidateStatus.STREET_NAME_MISSING
